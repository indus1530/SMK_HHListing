package edu.aku.hassannaqvi.smk_hhlisting_app.contracts;
...
public class VerticesContract {
    ...
    public VerticesContract sync(JSONObject jsonObject) throws JSONException {
        this.cluster_code = jsonObject.getString(VerticesTable.COLUMN_CLUSTER_CODE);
        this.poly_lat = jsonObject.getDouble(VerticesTable.COLUMN_POLY_LAT);
        this.poly_lng = jsonObject.getDouble(VerticesTable.COLUMN_POLY_LANG);
        this.poly_seq = jsonObject.getString(VerticesTable.COLUMN_POLY_SEQ);
    ...
    public VerticesContract hydrate(Cursor cursor) {
        this.cluster_code = cursor.getString(cursor.getColumnIndex(VerticesTable.COLUMN_CLUSTER_CODE));
        this.poly_lat = cursor.getDouble(cursor.getColumnIndex(VerticesTable.COLUMN_POLY_LAT));
        this.poly_lng = cursor.getDouble(cursor.getColumnIndex(VerticesTable.COLUMN_POLY_LANG));
        this.poly_seq = cursor.getString(cursor.getColumnIndex(VerticesTable.COLUMN_POLY_SEQ));
    ...
    public static abstract class VerticesTable implements BaseColumns {
